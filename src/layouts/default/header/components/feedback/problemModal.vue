<template>
  ...
  >
    <BasicForm @register="registerForm">
      <template #errorTime="{ model, field }">
        <DatePicker v-model:value="model[field]" show-time />
      </template>
...
<script setup lang="ts">
  ...
  import { problemFormSchema } from './feedback.data';
  import { addApi } from '/@/api/sys/sysFeedback';
  import { importFileFull } from '/@/components/System';
  import { message, Input, DatePicker } from 'ant-design-vue';
  ...
  const handleSubmit = async () => {
    try {
      ...
      //let times = newDate(values.errorTime._d);
      let attachments: any = [];
  ...
  const importExcel = async (e) => {
    try {
      ...
      fileList.value.push({ title: e.name, url: data.data.data });
