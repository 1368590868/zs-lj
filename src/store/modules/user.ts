...
import { getUserInfo, doLogout, UserLogin } from '/@/api/login/login';
...
export const useUserStore = defineStore({
  ...
  actions: {
    ...
    setHasCode(info: string | undefined) {
      this.hasCode = info;
    },
    ...
    setSystemConfig(List: []) {
      this.systemConfig = List;
    },
    setRoleClear(info) {
      this.roleClear = info;
    },
    ...
    async login(
      ...
      try {
        ...
        const data = await UserLogin(loginParams, mode);
        if (data.code != 1) {
