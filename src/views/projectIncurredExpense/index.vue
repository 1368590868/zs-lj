    ...
    <Card>
      <BasicForm class="form-wrap" @register="register">
        <template #costProject="{ model, field }">
          <ApiSelect :api="getProjectNameAndId" showSearch :filter-option="filterOption" placeholder="请选择"
            v-model:value="model[field]" resultField="data" labelField="projectName" valueField="id"
      ...
      <div v-show="showBar">
        ...
        <div class="flex justify-between flex-wrap mb-4 gap-4">
          <div class="flex">项目名称:<EllipsisText :maxWidth="200" :tooltipMaxWidth="200"
              :tooltip="projectDetail.projectName">
              {{ projectDetail.projectName }}
            </EllipsisText>
          </div>
          <div>项目预算：{{ useCurrencyFormatter(projectDetail.generalBudget) }} 元</div>
          <div>工程计划时间：{{ projectDetail.planStartDate }} - {{ projectDetail.planEndDate }}</div>
